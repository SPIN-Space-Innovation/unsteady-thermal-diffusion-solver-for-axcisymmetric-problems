...
#include "mesh.hpp"
#include "coef.hpp"
// Αναγκαία η εγκατάσταση της opensource βιβλιοθήλης Eigen (χρήσιμη για γραμμική άλγεβρα)
//#include "gmres.hpp"
...
class SteadyState{
    ...
    void fill(){
        ...
        int index;
        // Αρχικός μηδενισμός όλων των στοιχείων Αij
    ...
    void solver(string textname){
        VectorXd T_ = A_.fullPivLu().solve(B_); // Τ = [Α]^-1 * Β
        ofstream Temp;
        Temp.open(textname);
        for (int i = 0; i < N_; ++i) {
            for (int j = 0; j < N_; ++j) {
